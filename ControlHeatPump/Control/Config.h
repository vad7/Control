...
//		#define DEBUG_PID				// Отладка ПИДа
	...
	#define USE_SERIAL4							// Использовать порт Serial4 на D52(RXD2) и A11/D65(TXD2) +(X42.1-3.3V, X43.3-GND)
		...
		#define HEATER_HEAT_ON_MASK			1	// Маска определения, что котел работает на нагрев (dHeater.data.Status)
	#endif
...
//		#define PIN_DEVICE_RSUN_ON     53 //[R_7] Реле включение шарового крана солнечного коллектора
	...
	// Имена индексов ВАЖЕН ПОРЯДОК!
	#define RCOMP              0			// Реле включения компрессора (через пускатель)
	#define RPUMPO             1			// Реле включения насоса выходного контура  (отопление и/или ГВС), Обычное реле LOW LEVEL TRG через R_1k + PC817
//	#define RPUMPO_INVERT					// Инвертировать
//	#define RPUMPBH            2			// Реле насоса НАГРЕВА бойлера (ГВС) - не путать с циркуляцией
	#define R3WAY              2			// Трех ходовой кран Отопление - ГВС (Включение ГВС)
	#define R3WAYOFF           3			// Трех ходовой кран Отопление - ГВС (Выключение ГВС)
	#define RPUMPI             4			// Реле включения насоса входного контура  (геоконтур), Обычное реле LOW LEVEL TRG через R_1k + PC817
//	#define RPUMPI_INVERT					// Инвертировать
	#define RBOILER            5			// Включение ТЭНа бойлера
	#define R4WAY              6			// 4-ходовой реверсивный клапан (охлаждение)
	#define RPUMPFL            7			// Реле насоса Теплого Пола
	#define RBATH              8			// Реле включения отопителя ванной
