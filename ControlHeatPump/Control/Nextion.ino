				...
				if(cmd1 == NXTID_PAGE_MAIN) {
					...
					if(cmd2 == NXTID_BOILER_ONOFF) {                       // событие нажатие кнопки вкл/выкл ГВС
						...
						if(GETBIT(HP.Prof.Boiler.flags, fAddHeatElement)) {// режим догрева
							HP.Prof.Boiler.TempHeatElement += cmd2 == NXTID_BOILER_PLUS ? 100 : -100;
							if(HP.Prof.Boiler.TempHeatElement < 1000) HP.Prof.Boiler.TempHeatElement = 1000;
							else if(HP.Prof.Boiler.TempHeatElement > HP.Prof.Boiler.TempTarget - HP.Prof.Boiler.dTemp) HP.Prof.Boiler.TempHeatElement = HP.Prof.Boiler.TempTarget - HP.Prof.Boiler.dTemp;
							dptoa(ntemp, HP.Prof.Boiler.TempHeatElement / 10, 1);
						} else {
...
void Nextion::Update()
{
	...
	} else if(PageID == NXTID_PAGE_BOILER)  // Обновление данных 6 страницы "ГВС"
	{
		...
		strcat(dptoa(ntemp, HP.sTemp[TCONING].get_Temp() / 10, 1), NEXTION_xB0);
		setComponentText("tconing", ntemp);
		dptoa(ntemp, (GETBIT(HP.Prof.Boiler.flags, fAddHeatElement) ? HP.Prof.Boiler.TempHeatElement : HP.get_boilerTempTarget()) / 10, 1);
		setComponentText("t", ntemp);
