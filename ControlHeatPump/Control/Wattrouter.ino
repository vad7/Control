...
void WR_ReadPowerMeter(void)
{
	if(GETBIT(HP.Option.flags, fBackupPower)) {
		...
		int8_t i = devModbus::Process(WR_PowerMeter_Modbus, WR_PowerMeter_ModbusReg, (int16_t*)&WR_PowerMeter_Power, READ_INPUT);
	#else
		int8_t i = devModbus::Process(WR_PowerMeter_Modbus, WR_PowerMeter_ModbusReg, (uint32_t*)&WR_PowerMeter_Power, READ_INPUT_CDAB);
	#endif
		if(i == OK) {
