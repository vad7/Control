					...
					case HTTP_GET:     // чтение файла
					{
						...
						if(GETBIT(Socket[thread].flags, fUser)) {
							if(strstr(Socket[thread].inPtr, ".html")) {
								...
								|| strcmp(Socket[thread].inPtr, "charts.html") == 0
			...
			} else if (strcmp(str,"FC")==0)    // RESET_FC
			{
				if (!HP.dFC.get_present()) {
					...
					if(HP.dFC.reset_FC()) {                            // подать команду на сброс
						strcat(strReturn, "OK");
#if defined(FC_VACON) && defined(FC_POWER_IN_PERCENT)
		...
		if (((x=strpbrk(str,"("))!=0)&&((y=strpbrk(str,")"))!=0))  // Функция с одним параметром - найдена открывающиеся и закрывающиеся скобка
		{
			...
			*x++ = 0;			// '(' -> '\0'
			...
			if (strcmp(str,"get_pFC")==0)           // Функция get_pFC - получить значение параметра FC
			{
				...
				if(strncmp(x, "INFO", 4)==0) {
					x += 4;
#ifndef FC_ANALOG_CONTROL
					SemaphoreGive(xWebThreadSemaphore);  // Мютекс веба отдать
					if(*x == '2') {
						l_i32 = HP.dFC.read_0x03_32(FC_POWER_DAYS); // +FC_POWER_HOURS (low word)
...
xget_WR:
							if(p == 0) { // get_WR(0)
								...
								if(WR_Pnet == -32768) strcat(strReturn, "-");
								else {
