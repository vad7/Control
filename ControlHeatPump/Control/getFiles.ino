...
// используется для получения состояния ТН в виде txt и получения сосирянмя при отсылке по почте
void get_Counts(char *tempBuf)
{

       strcpy(tempBuf,"\n  Сохраненные в ЕЕПРОМ значения внутренних счетчиков\r\n");
       strcat(tempBuf,"-- Глобальные счетчики (Всего за весь период) --\r\n");
...
//           strcat(Socket[thread].outBuf,"Суммарная активная энергия [кВт/ч]: ");                     HP.dSDM.get_paramSDM((char*)sdm_ACENERGY,Socket[thread].outBuf); STR_END;
  ...
  get_Counts(Socket[thread].outBuf); // Счетчики
  sendBufferRTOS(thread,(byte*)Socket[thread].outBuf,strlen(Socket[thread].outBuf));     
...
void get_txtSettings(uint8_t thread, char *filename)
{
     ...
     strcat(Socket[thread].outBuf,"Значение температуры для включения догрева бойлера (C°): "); HP.Prof.get_boiler((char*)boil_TempHeatElement,Socket[thread].outBuf) ;STR_END;
     ...
     strcat(Socket[thread].outBuf,"Включать догрев, если компрессор не нагрел бойлер до температуры догрева: ");HP.Prof.get_boiler((char*)boil_fAddHeatElementForce,Socket[thread].outBuf);STR_END;
     ...
     if((HP.get_flags() & (1<<f1Wire1TSngl))) strcat(Socket[thread].outBuf, "1 ");
     if((HP.get_flags() & (1<<f1Wire2TSngl))) strcat(Socket[thread].outBuf, "2 ");
     if((HP.get_flags() & (1<<f1Wire3TSngl))) strcat(Socket[thread].outBuf, "3 ");
     if((HP.get_flags() & (1<<f1Wire4TSngl))) strcat(Socket[thread].outBuf, "4");
     sendBufferRTOS(thread,(byte*)Socket[thread].outBuf,strlen(Socket[thread].outBuf));  
...
void get_mailState(EthernetClient client,char *tempBuf)
{
	...
	strcat(tempBuf,cStrEnd);  client.write(tempBuf,strlen(tempBuf));
	if(HP.dFC.get_present()) {
		strcpy(tempBuf,"Текущее состояние инвертора: "); _itoa(HP.dFC.get_state(),tempBuf);
		strcat(tempBuf,cStrEnd);
		client.write(tempBuf,strlen(tempBuf));
