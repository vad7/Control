...
boolean HeatPump::set_optionHP(char *var, float x)   
{
	...
	if(strcmp(var,option_f2LogExtendedInfo)==0)	{ Option.flags2 = (Option.flags2 & ~(1<<f2LogExtendedInfo)) | ((n!=0)<<f2LogExtendedInfo); return true; } else
...
char* HeatPump::get_optionHP(char *var, char *ret)
{
	...
	if(strcmp(var,option_f2LogExtendedInfo)==0){ return strcat(ret, (char*)(GETBIT(Option.flags2, f2LogExtendedInfo) ? cOne : cZero)); } else
...
//#endif
	...
	Pump_HeatFloor(!b);
#else // Нет трехходового - схема с двумя насосами с обратными клапанами
...
void HeatPump::Pumps(bool b)
{
	...
	if(b) { // ВКЛ
		...
		if(GETBIT(dRelay[PUMP_IN].flags, fR_StatusMain) || (error && dRelay[PUMP_IN].get_Relay())) {
			...
			if(GETBIT(Option.flags2, f2modWorkLog)) journal.jprintf(" Stop IN pump in %d s\n", delayed);
	...
	if(b || (!b && (!get_workPump() || get_pausePump() || get_modeHouse() == pOFF))) Pump_HeatFloor(b); // насос ТП
   	...
   	if(b || (!b && (!get_workPump() || get_pausePump() || get_modeHouse() == pOFF))) {
   		dRelay[PUMP_OUT].set_Relay(b);                // насос отопления
   	   	_delay(DELAY_AFTER_SWITCH_RELAY);									// Задержка на d мсек
...
void HeatPump::StartResume(bool start)
{
	if(NO_Power) {
...
void HeatPump::StopWait(bool stop)
{
	if(stop) {
		...
		if(get_State() == pOFF_HP || get_State() == pWAIT_HP || get_State() == pSTOPING_HP) return; // Если ТН выключен или выключается или ожидание ничего не делаем
...
#ifdef DEBUG_MODWORK
			journal.printf_time(" StateHP: %d, fRB=%d\n", get_State(), flagRBOILER);
#endif
			dRelay[RBOILER].set_OFF();
		}
		if(get_State() == pOFF_HP || get_State() == pSTOPING_HP || get_State() == pWAIT_HP) { // Если ТН выключен или выключается
...
MODE_COMP HeatPump::UpdateCool()
{
	...
	case pHYSTERESIS:  // Гистерезис охлаждение.
		if(t1 > target + (GETBIT(HP.Option.flags, fBackupPower) ? Prof.Cool.dTempGen : Prof.Cool.dTemp) && !_is_compr_on) { Status.ret=pCh2; return pCOMP_ON; } // Достигнут гистерезис ВКЛ
	...
	//    		if((FEED<(targetRealPID*(100+(100-dFC.get_PidStop()))/100)))                                                                    {Status.ret=pCp17; resetPID(); return pCOMP_NONE;}   // Подача ограничение, с учетом погодозависимости Подход сверху
			...
			return pCOMP_NONE;                                                                            // компрессор состояние не меняет
		} else { Status.ret=pCp5; return pCOMP_NONE; }             // Если компрессор не работает то ничего не делаем и выходим
		break;
	case pHYBRID:
		set_Error(ERR_WRONG_HARD_STATE, (char*)__FUNCTION__);	// не реализовано
		break;
...
#ifdef DEBUG_MODWORK
	journal.printf_time("get_Work-Boiler: %s(%s), ret=%X, onBoiler=%d, flagRBOILER=%d\n", codeRet[Status.ret], codeRet[Status.prev], ret, onBoiler, flagRBOILER);
#endif
...
//#ifdef DEBUG_MODWORK
//	journal.printf_time("Pause %d, mW:%d\n", pause, Status.modWork);
//#endif
	if(heater) {
