...
#define fBoilerON 				1		// флаг Греть бойлер
...
#define fScheduleHeatElement 0			// флаг Расписание только для ТЭНа
...
#define fAddHeatElement     6           // флаг ДОГРЕВА ГВС ТЭНом
#define fBoilerTogetherHeat	7			// флаг одновременного нагрева бойлера с отоплением до температуры догрева
#define fBoilerHeatElementOnly 8		// флаг нагрева только ТЭНом, при этом если не по рассписанию только для тэна, то берется температура TempHeatElement
...
struct type_boilerHP {
	...
	int16_t TempTarget;                // Целевая температура бойлера, сотые градуса
	int16_t dTemp;                     // гистерезис целевой температуры, сотые градуса
	int16_t TempOutLim;                // Tемпература подачи максимальная, сотые градуса
	...
	int16_t dHeatElement;              // Гистерезис нагрева бойлера до температуры догрева, в сотых градуса
	int16_t TempHeatElement;           // Температура догрева бойлера
	...
	int16_t tempPID;                   // Целевая температура подачи, сотые градуса
...
// Работа с отдельными флагами type_setting_*.flags
...
struct type_setting_heat {
	...
	uint8_t _reserved_;				//
	int16_t Temp1;					// Целевая температура дома, сотые
	int16_t Temp2;					// Целевая температура обратки, сотые
	int16_t dTemp;					// Гистерезис целевой температуры, сотые
	uint16_t pid_time;				// Период расчета ПИД в секундах
	PID_STRUCT pid;					// Настройки и переменные ПИД регулятора
	int16_t dTempGen;				// Гистерезис целевой температуры от генератора
	int16_t TempOutLim;				// Tемпература подачи (минимальная для охлаждения или максимальная для нагрева), сотые
	int16_t TempInLim;				// Tемпература обратки для включения (максимальная для охлаждения или минимальная для нагрева), сотые
	uint16_t WorkPause;				// Минимальное время простоя, секунды
	...
	uint8_t add_delta_end_hour;		// Конечный Час добавки температуры к установке
	int16_t tempPID;				// Целевая темперaтура подачи, сотые градуса
...
struct type_setting_cool {
	...
	PID_STRUCT pid;					// Настройки и переменные ПИД регулятора
	int16_t dTempGen;				// Гистерезис целевой температуры от генератора
	int16_t TempOutLim;				// Tемпература подачи (минимальная для охлажления или максимальная для нагрева)
	int16_t TempInLim;				// Tемпература обратки (максимальная для охлажления или минимальная для нагрева)
	uint16_t WorkPause;				// Минимальное время простоя, секунды
	int16_t MaxDeltaTempOut;		// Максимальная разность температур входа-выхода отопления.
	int16_t kWeatherPID;			// Коэффициент погодозависимости PID, в ТЫСЯЧНЫХ градуса на градус
	int8_t WeatherBase;				// Базовая температура для погодозависимости, градусы
