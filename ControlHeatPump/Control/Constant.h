...
#endif
#define SerialDbg (*dbgPtr)
Stream* dbgPtr = &Serial;
...
#define WEB_MAX_LOCK_TIME 10000UL			// максимальное время не выполенния web_server() для сброса семафора, мсек

#define DISK_TIME_WAIT		1000			// Время ожидания семафора SD/Flash
// ------------------- SERIAL --------------------------------
//Nextion дисплей
...
#define I2C_ADR_ADS1115      0x48		// I2C ADC
#define I2C_ADR_ADS1115_2    0x49		// I2C ADC

// --------------------------------------------------------------------------------------------------------------------------
#if I2C_SIZE_EEPROM >= 64 //I2C_EEPROM_64KB
// Стартовые адреса -----------------------------------------------------
...
// 0x0A00 - I2C_PROFILE_EEPROM хранение профилей, максимальный размер 0x1000 (4096) байт. Число профилей 10 шт. Размер профиля сейчас 341 байт.
...
// Меньше 8 кбайт
// 0х0000 - I2C_COUNT_EEPROM хранение счетчиков, максимальный размер 0x79 (127) байт. Сейчас используется 52 байта
// 0х0080 - I2C_SETTING_EEPROM хранение настроек ТН, максимальный размер 0х680 (1408) байт.
// 0х0700 - I2C_PROFILE_EEPROM хранение профилей, максимальный размер 0x760 (1888) байт.
...
#define MIN_WEATHER       (3*100)        // Минимальная температура подачи при погодозависимости
...
#define HYSTERESIS_BoilerTogetherHeatSt	700	// Гистерезис совместного нагрева бойлера с отоплением в сотых градуса, нагрев
#define HYSTERESIS_BoilerTogetherHeatEn	350	// Гистерезис совместного нагрева бойлера с отоплением в сотых градуса, останов нагрева
#define HYSTERESIS_BoilerAddHeat      	300	// Гистерезис нагрева бойлера до температуры догрева, в сотых градуса
#define HYSTERESIS_HeatFloor		 	25	// Гистерезис раздельного управления реле теплого пола
#define HeatFloorDeltaTemp			 	350	// Добавка к целевой температуре для раздельного управления реле теплого пола, полученная температура будет целевой для ТП
#define LEGIONELLA_DAY    3              // День когда включается алгоритм обеззараживания воды (Понедельник 1 воскресенье 7)
#define LEGIONELLA_HOUR   1              // Час когда включается алгоритм обеззараживания воды (должно быть 0 минут)
...
#endif
#ifndef WEB0_FREQUENT_JOB_PERIOD
#define WEB0_FREQUENT_JOB_PERIOD 1000	 // Периодичность важных функций в задаче WEB0, мс
#endif
#ifndef WEB0_OTHER_JOB_PERIOD
#define WEB0_OTHER_JOB_PERIOD   10*1000  // Периодичность других функций внутри задаче WEB0, мс
#endif
#ifndef WEB0_FREQUENT_JOB_PERIOD
#define WEB0_FREQUENT_JOB_PERIOD 1000	 // Периодичность важных функций в задаче WEB0, мс
#endif
...
const char *option_f2NextionLog       = {"NL"};
...
const char *option_f2LogExtendedInfo  = {"EL"};
...
const char *hp_TempOutLim= {"TOUT"};            // температура подачи (минимальная для охлажления или максимальная для нагрева)
const char *hp_TempInLim = {"TIN"};             // температура обратки (максимальная для охлажления или минимальная для нагрева)
...
const char *boil_TempOutLim   = {"TOUT"};                // Tемпература подачи максимальная
...
const char *WHeater_fHeater_BoilerModeByExtTemp	= {"BH"};
const char *WHeater_fHeater_DontSetFlowTemp	    = {"NF"};
const char *WHeater_fHeater_Log				    = {"JL"};
...
const char *WHeater_HeatingPipesAddTemp			= {"HPT"};
const char *WHeater_HeatFloorAddTemp			= {"HFT"};
...
#define ERR_DTEMP_CON      -26         // Превышена разность температур выхода-входа отопления
#define ERR_DTEMP_EVA      -27         // Превышена разность температур на испарителе
...
#define ERR_MODBUS_0xe3      -48         // Modbus 0xe3 Master invalid response CRC exception
  ...
  #define ERR_MODBUS_VACON_TEMP -56       // Перегрев радиатора инвертора
#endif
...
const char *noteError[] = {"Ok",                                                  //  0
                           ...
                           "Адрес датчика температуры не установлен",             // -6
                           ...
                           "FreeRTOS не может создать задачу, не хватает пямяти", // -13
                           "Отсутствует датчик давления, и выбран алгоритм ЭРВ без него",// -14 Длина 132 байта
                           ...
                           "Превышена разность температур выхода-входа отопления",// -26
                           "Превышена разность температур геоконтура",            // -27
                           ...
                           "Слишком большое напряжение сети (данные электросчетчика)",                             //-75
                           "Слишком большая потребляемая мощность (данные электросчетчика)",                       //-76
                           "Modbus требуется, но отсутствует в конфигурации",                                     //-77
...
// 13 - включение по обратке - достигнута мин/макс температура обратки
...
enum MESSAGE          
{
  ...
  pMESSAGE_EXT_WARNING,          // 9 Уведомление "Внешние уведомления (BMS)"
